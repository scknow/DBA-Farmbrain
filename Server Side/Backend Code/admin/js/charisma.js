...
function docReady(){
	...
	var table=$('.datatable').dataTable({
			...
			"sDom": "<<'filters-set''H'<'myTools'><'myTools1'><'myTools4'><'myTools5'><'myTools2'><'myTools3'>><'row-fluid'<'span6'><'span6'f>r>t<'row-fluid'<'span14'i>l><'span12 center'p>>",
...
function finish_loading_filters(){
	$(".myTools2 i").on("click", function(e){
		$(".myTools2 input").val('').change();
	});
	$(".myTools3 i").on("click", function(e){
		$(".myTools3 input").val('').change();
	});
	// var d = new Date();
	...
	// $("#EndD").rules('add', { greaterThan: "#startD" });
}

function check_record_selected(chklist){
	if(chklist.length===0){
		return false;
	}
	return true;
...
function download_csv_selected(type, checklist, page_name, action, type_nam){
	if(!check_record_selected(checklist)){
		alert("Please select atleast one record to download.");
	}else{
		location.href="dba_export_csv.php?type="+type+"&list="+checklist+"&page_name="+page_name+"&action="+action+"&type_nam="+type_nam;
	}
}