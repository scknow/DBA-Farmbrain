	...
	.done(function( msg ){
		...
		for(i=0;i<price_obj.length;i++)
		{
			...
			if(row.productid==pid_new)
			{
				...
				{
					//alert(row.productid)
					presup_for_add_new(pidxof,row.supplierid)
					//presup_glob_select(pidxof,row.supplierid)
				}
...
function presup_glob_select(index,sid)
{
	for(var i=0;i<final_list.length;i++)
	{
		var global_suplier1=new Array();
		...
		if(indx!=-1 && ($("#"+id_glob).prop("checked") == true))
		{
			...
			{
				var rvix = removed_supplier_index[indx].sid.indexOf(sid);
				for(var iik=0;iik<removed_supplier_index[indx].sid.length;iik++)
				{
					if(removed_supplier_index[indx].sid[iik]==sid)
					{
						rvix=iik;
					}
				}
...
if(button==2)
{
	...
	for(k=0;k<sup_obj.length;k++){
		...
		{	
			...
			var pid = parseInt(sup_obj[k].supplierid);
			var idx = global_suplier.indexOf(pid);
			var prvid=k+"prem"+sup_obj[k].supplierid;
			if(idx!=-1)
			{
				presup_glob_select(k,sid)
			}
			//presup(k,sid);
		}
	}
