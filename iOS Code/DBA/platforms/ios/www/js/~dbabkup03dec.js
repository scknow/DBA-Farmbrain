...
function cr()
{
	if($("#firstname").val()=='' || $("#lastname").val()=='' ||  $("#eml").val()=='' || $("#bill_ad1").val()==''|| $("#bill_zip").val()=='' || $("#bill_phone").val()=='' )
	{
...
/***********************New variable form jatin*****************************/
var product_new=new Array();
var supplier_new=new Array();
var price_new=new Array();
...
function place_supplier(sid)
{
	...
	for(i=0;i<final_list.length;i++){
		...
		{
		//alert("in");
			if(final_list[i].qty!=0 && final_list[i].qty!='NAN')
			{
...
function get_pricing_test(pid_new)
{
	...
	.done(function( msg ){
		...
		for(i=0;i<price_obj.length;i++)
		{
			...
			var in_glob=global_suplier.indexOf(row.supplierid)
			for(var ki=0;ki<global_suplier.length;ki++)
			{
				if(global_suplier[ki]==row.supplierid)
				{
					in_glob=ki;
				}
			}			
			
			if(row.productid==pid_new && in_glob!=-1)
			{
				var indx = price_index.indexOf(row.productid);
				var pidxof = product_index.indexOf(row.productid);
				//alert(final_list[pidxof]);
			...
			else if(in_glob==-1 && row.productid==pid_new)
			{
				var indx = price_index.indexOf(row.productid);
				var pidxof = product_index.indexOf(row.productid);
				//alert(final_list[pidxof]);				
				...
				if(indx!=-1)
				{
					...
					if(kxd!=-1)
					{
						...
						var rindx = removed_product_index.indexOf(row.productid);
						if(rindx==-1)
						{
							var sidg = new Array();
							var padt = new Array();
						...
						{
							removed_supplier_index[rindx].sid.push(row.productid);
...
function sup_global_setng(button)
{
	if(button==2){
		...
		for(k=0;k<sup_obj.length;k++){
			...
			if(idx!=-1)
			{
				str = str + "<div class='ss-supplier'><div class='ss-supplier-selector'><input type='checkbox' id='glob_"+sup_obj[k].supplierid+"' checked onclick='presup1("+sup_obj[k].supplierid+")' /></div><div class='ss-supplier-label'>"+sup_obj[k].businessname+"</div><div class='ss-supplier-percentage' ><input type='number' value='"+global_premium[k]+"' id='gprem"+sup_obj[k].supplierid+"' onkeyup='presup2("+k+","+sup_obj[k].supplierid+")' /> %</div></div>";

			}else{
				str = str + "<div class='ss-supplier'><div class='ss-supplier-selector'><input type='checkbox' id='glob_"+sup_obj[k].supplierid+"' onclick='presup1("+sup_obj[k].supplierid+")' /></div><div class='ss-supplier-label'>"+sup_obj[k].businessname+"</div><div class='ss-supplier-percentage' ><input type='number' id='gprem"+sup_obj[k].supplierid+"' value='0' onkeyup='presup2("+k+","+sup_obj[k].supplierid+")' /> %</div></div>";
			}
		}
			$("#sup_list_view_global").html(str);		
			$('.overlay').fadeIn(100); 
			$('#setng_supplier_global').fadeIn(100);
			for(var k=0;k<sup_obj.length;k++)
			{
				var sup_id_all=sup_obj[k].supplierid;			
				apply_global_setting(sup_id_all);
			}
	}
}
function apply_global_setting(sid1)
{
	for(var i=0;i<final_list.length;i++)
	{
		...
		var id_glob="glob_"+sid1;
		if(indx!=-1 && ($("#"+id_glob).prop("checked") == true))
		{
			var prinx = price_index.indexOf(final_list[i].id);			
			if(prinx!=-1)
			{
					var rvix = removed_supplier_index[indx].sid.indexOf(sid1);
					if(rvix!=-1)
					{
			...
			else
			{
				...
				var rvix = removed_supplier_index[indx].sid.indexOf(sid1);
				for(var hj=0;hj<removed_supplier_index[indx].sid.length;hj++)
				{
					if(removed_supplier_index[indx].sid[hj]==sid1)
					{
						rvix=hj;
					}
				}		
		...
		else if($("#"+id_glob).prop("checked") == false)
		{
			...
			if(indx!=-1)
				{
					...
					if(kxd!=-1)
					{
						...
						{
							removed_supplier_index[rindx].sid.push(sid1);
							removed_supplier_index[rindx].pda.push(price_data);
						}
						price_values[indx].id.splice(kxd,1);
						price_values[indx].price.splice(kxd,1);
