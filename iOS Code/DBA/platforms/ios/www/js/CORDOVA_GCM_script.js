...
document.addEventListener('deviceready', function() {
 ...
 // $("#app-status-ul").append('<li>deviceready event received</li>' );
  ...
  //
	window.plugins.GCM.register("583235237590", "GCM_Event", GCM_Success, GCM_Fail );
  ...
  case 'registered':
    ...
    if ( gApp.gcmregid.length > 0 )
    {
     //alert(e.regid);
		reg_id=	e.regid;
		name='tab1';
		email='jatin1414@gmail.com';
		
		$.ajax({
			type: "POST",
			url: base_url+"register_gcm.php",
			data: {gcm:reg_id, usr:userid, os:"android"}
		})
		.done(function( msg ){
			//alert(msg);
		});
